...
class CoefficientMappingModule(torch.nn.Module):
    """Define Class CoefficientMappingModule."""
    ...
    def get_rotate_inv_rescale(self, lmax, mmax):
        """Run get_rotate_inv_rescale method."""
        # code.
...
class SO3_Rotation(torch.nn.Module):
    ...
    def RotationToWignerDMatrix(self, edge_rot_mat, start_lmax, end_lmax):
        """Run RotationToWignerDMatrix method."""
        # code.
    ...
    def __init__(
        ...
        if lmax != mmax:
            for l in range(lmax + 1):
                ...
                length = 2 * l + 1
                rescale_factor = math.sqrt(length / (2 * mmax + 1))
                to_grid_mat[:, :, start_idx: (start_idx + length)] = to_grid_mat[:, :,
                                                                     start_idx: (start_idx + length)] * rescale_factor
        ...
        if lmax != mmax:
            for l in range(lmax + 1):
                ...
                length = 2 * l + 1
                rescale_factor = math.sqrt(length / (2 * mmax + 1))
                from_grid_mat[:, :, start_idx: (start_idx + length)] = from_grid_mat[:, :,
                                                                       start_idx: (start_idx + length)] * rescale_factor
