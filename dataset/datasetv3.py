...
def collate_batch_features(batch_data, max_num_nn=64):
    ...
    # code.
    X_0 = torch.cat([data['X_0'] for data in batch_data], dim=0)
    X_1 = torch.cat([data['X_1'] for data in batch_data], dim=0)
    q_0 = torch.cat([data['q_0'] for data in batch_data], dim=0)
    q_1 = torch.cat([data['q_1'] for data in batch_data], dim=0)

    sizes0 = torch.tensor([data['M_0'].shape for data in batch_data])
    sizes1 = torch.tensor([data['M_1'].shape for data in batch_data])

    ids_topk_0 = torch.zeros((X_0.shape[0], max_num_nn), dtype=torch.long, device=X_0.device)
    ids_topk_1 = torch.zeros((X_1.shape[0], max_num_nn), dtype=torch.long, device=X_1.device)
    M_0 = torch.zeros(torch.Size(torch.sum(sizes0, dim=0)), dtype=torch.float, device=X_0.device)
    M_1 = torch.zeros(torch.Size(torch.sum(sizes1, dim=0)), dtype=torch.float, device=X_1.device)

    batch_mask_0 = torch.zeros(X_0.shape[0], dtype=torch.int, device=X_0.device)
    batch_mask_1 = torch.zeros(X_1.shape[0], dtype=torch.int, device=X_1.device)

    for i, size, data in zip(range(len(batch_data)), torch.cumsum(sizes0, dim=0), batch_data):
        ix1 = size[0]
        ix0 = ix1 - data['M_0'].shape[0]
        iy1 = size[1]
        iy0 = iy1 - data['M_0'].shape[1]
        ids_topk_0[ix0:ix1, :data['ids_topk_0'].shape[1]] = data['ids_topk_0'] + ix0 + 1
        M_0[ix0:ix1, iy0:iy1] = data['M_0']

        batch_mask_0[ix0:ix1] = i

    for i, size, data in zip(range(len(batch_data)), torch.cumsum(sizes1, dim=0), batch_data):
        ix1 = size[0]
        ix0 = ix1 - data['M_1'].shape[0]
        iy1 = size[1]
        iy0 = iy1 - data['M_1'].shape[1]
        ids_topk_1[ix0:ix1, :data['ids_topk_1'].shape[1]] = data['ids_topk_1'] + ix0 + 1
        M_1[ix0:ix1, iy0:iy1] = data['M_1']

        batch_mask_1[ix0:ix1] = i

    residue_mask_0 = get_residue_mask(M_0, batch_mask_0)
    residue_mask_1 = get_residue_mask(M_1, batch_mask_1)

    feats = {
        'X_0': X_0,
        'X_1': X_1,
        'ids_topk_0': ids_topk_0,
        'ids_topk_1': ids_topk_1,
        'q_0': q_0,
        'q_1': q_1,
        'M_0': M_0,
        'M_1': M_1,
        'residue_mask_0': residue_mask_0,
        'residue_mask_1': residue_mask_1,
    }

    if "id_f_0" in batch_data[0]:
        id_f_0 = torch.cat([data['id_f_0'] for data in batch_data], dim=0)
        id_f_1 = torch.cat([data['id_f_1'] for data in batch_data], dim=0)
        feats.update({"id_f_0": id_f_0})
        feats.update({"id_f_1": id_f_1})
    return feats
...
def collate_batch_data(batch_data):
    ...
    feats['y_0'] = torch.cat([torch.tensor([data['quality_0'], ], dtype=torch.float) for data in batch_data])
    feats['y_1'] = torch.cat([torch.tensor([data['quality_1'], ], dtype=torch.float) for data in batch_data])
    feats['rmsd_ca_95_0'] = torch.cat([torch.tensor([data['rmsd_ca_95_0'], ], dtype=torch.float) for data in batch_data])
    feats['rmsd_ca_95_1'] = torch.cat([torch.tensor([data['rmsd_ca_95_1'], ], dtype=torch.float) for data in batch_data])
    feats['rmsd_ca_0'] = torch.cat([torch.tensor([data['rmsd_ca_0'], ], dtype=torch.float) for data in batch_data])
    feats['rmsd_ca_1'] = torch.cat([torch.tensor([data['rmsd_ca_1'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_95_0'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_95_0'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_95_1'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_95_1'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_0'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_0'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_1'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_1'], ], dtype=torch.float) for data in batch_data])
    feats['pdb_0'] = [data['pdb_0'] for data in batch_data]
    feats['pdb_1'] = [data['pdb_1'] for data in batch_data]

    feats['rmsd_ca_95_scale_0'] = torch.cat([torch.tensor([data['rmsd_ca_95_scale_0'], ], dtype=torch.float) for data in batch_data])
    feats['rmsd_ca_95_scale_1'] = torch.cat([torch.tensor([data['rmsd_ca_95_scale_1'], ], dtype=torch.float) for data in batch_data])
    feats['rmsd_ca_scale_0'] = torch.cat([torch.tensor([data['rmsd_ca_scale_0'], ], dtype=torch.float) for data in batch_data])
    feats['rmsd_ca_scale_1'] = torch.cat([torch.tensor([data['rmsd_ca_scale_1'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_95_scale_0'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_95_scale_0'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_95_scale_1'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_95_scale_1'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_scale_0'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_scale_0'], ], dtype=torch.float) for data in batch_data])
    feats['cdr3_rmsd_ca_scale_1'] = torch.cat(
        [torch.tensor([data['cdr3_rmsd_ca_scale_1'], ], dtype=torch.float) for data in batch_data])
    feats['loss_weight_0'] = torch.cat(
        [torch.tensor([data['loss_weight_0'], ], dtype=torch.float) for data in batch_data])
    feats['loss_weight_1'] = torch.cat(
        [torch.tensor([data['loss_weight_1'], ], dtype=torch.float) for data in batch_data])
    return feats
...
class Dataset(torch.utils.data.Dataset):
    ...
    def merge_data(self,data0, data1):
        ...
        for key in data0.keys():
            new_data[f"{key}_0"] = data0[key]

        for key in data1.keys():
            new_data[f"{key}_1"] = data1[key]
